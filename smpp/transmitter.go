...
import (
	...
	"github.com/Zensey/go-smpp/smpp/pdu/pdutext"
)
...
func (t *Transmitter) SubmitLongMsg(sm *ShortMessage) (*ShortMessage, error) {
	//maxLen := 134 // 140-7(6) (UDH with 2(1) byte reference number)
	//if sm.Text.Type() == pdutext.UCS2Type {
	//	maxLen = 132 // 132 -- to avoid a character being split between payloads
	//}
	maxLen := 254 - 6 // Many smpp gateways allow long short message, 254 byte long
	rawMsg := sm.Text.Encode()
	countParts := int((len(rawMsg)-1)/maxLen) + 1

	t.rMutex.Lock()
	rn := uint8(t.r.Intn(0xFF))
	t.rMutex.Unlock()
	UDHHeader := make([]byte, 6)
	UDHHeader[0] = 0x05              // length of user data header
	UDHHeader[1] = 0x00              // information element identifier, CSMS 16 bit reference number
